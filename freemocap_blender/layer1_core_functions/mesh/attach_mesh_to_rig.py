...
from freemocap_blender.layer1_core_functions.mesh.create_mesh.create_mesh import create_mesh
...
def create_custom_mesh_og(rig):
    ...
    # Rename the trunk mesh to freemocap_mesh
    body_meshes[0].name = "freemocap_mesh"
    # Deselect all
    bpy.ops.object.select_all(action='DESELECT')
    # Select all body meshes
    for body_mesh in body_meshes:
        body_mesh.select_set(True)
    # Set freemocap_mesh as active
    bpy.context.view_layer.objects.active = body_meshes[0]
    # Join the body meshes
    bpy.ops.object.join()
    ### Parent the freemocap_mesh with the rig
    # Select the rig
    rig.select_set(True)
    # Set rig as active

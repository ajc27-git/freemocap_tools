...
def add_rig(use_limit_rotation: bool=False):
    ...
    try:
        ...
        for object in bpy.data.objects:
            print(str(object.name) + ': ' + str(object.type))
...
class VIEW3D_PT_freemocap_adapter(Panel):
    ...
    def draw(self, context):
        ...
        box.operator('fmc_adapter.actions_op', text='Adjust Empties').action = 'ADJUST_EMPTIES'
        ...
        box.operator('fmc_adapter.actions_op', text='Add Rig').action = 'ADD_RIG'
...
# Operator class that executes the methods
class FMC_ADAPTER_OT_actions(Operator):
    bl_idname       = 'fmc_adapter.actions_op'
    bl_label        = 'Freemocap Adapter'
    bl_description  = 'Run Function'
    bl_options      = {'REGISTER', 'UNDO_GROUPED'}
    
    action: EnumProperty(
        items=[
            ('ADJUST_EMPTIES', 'adjust empties', 'adjust empties'),
            ('ADD_RIG', 'add rig', 'add rig'),
            ('ADD_BODY_MESH', 'add body mesh', 'add body mesh')
        ]
    )
    ...
    def execute(self, context):
        ...
        if self.action == 'ADJUST_EMPTIES':
            self.adjust_empties(context, fmc_adapter_tool)
        elif self.action == 'ADD_RIG':
            self.add_rig(context, fmc_adapter_tool)
        elif self.action == 'ADD_BODY_MESH':
            self.add_body_mesh(context, fmc_adapter_tool)
        return {'FINISHED'}
    
    @staticmethod
    def adjust_empties(context, fmc_adapter_tool):
    ...
    @staticmethod
    def add_rig(context, fmc_adapter_tool):
    ...
    @staticmethod
    def add_body_mesh(context, fmc_adapter_tool):
        # Execute Adjust Empties first
        adjust_empties(z_align_ref_empty=fmc_adapter_tool.vertical_align_reference,
                       z_align_angle_offset=fmc_adapter_tool.vertical_align_angle_offset,
                       ground_ref_empty=fmc_adapter_tool.ground_align_reference,
...
classes = [FMC_ADAPTER_PROPERTIES, VIEW3D_PT_freemocap_adapter, FMC_ADAPTER_OT_actions]

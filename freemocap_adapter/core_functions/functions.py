    ...
    if correct_fingers_empties:
        ...
        for frame in range(scene.frame_start, scene.frame_end):
            ...
            for side in hand_side:
                ...
                translate_empty(EMPTIES_HEIRARCHY_DICT, side + '_hand_wrist', frame, position_delta)
    ...
    for bone in VIRTUAL_BONES:
        ...
        for length in VIRTUAL_BONES[bone]['lengths']:
            ...
            if length < inferior_limit or length > superior_limit:
                ...
                translate_empty(EMPTIES_HEIRARCHY_DICT, tail, frame_index, position_delta)
...
def add_mesh_to_rig(body_mesh_mode: str = "custom"):
    if body_mesh_mode == "file":
        ...
        shoulder_R.tail[0] - right_arm_mesh_depth / 2, shoulder_R.tail[1], shoulder_R.tail[2] - 0.02)
        right_arm_mesh_radius = right_arm_mesh_depth / 10
        left_arm_mesh_depth = hand_L.head[0] - shoulder_L.tail[0]
        left_arm_mesh_location = (
        shoulder_L.tail[0] + left_arm_mesh_depth / 2, shoulder_L.tail[1], shoulder_L.tail[2] - 0.02)
